...
CREATE database if NOT EXISTS `Voucher` default character set utf8mb4 collate utf8mb4_0900_ai_ci;
use `Voucher`;
...
CREATE TABLE `tb_voucher_order`  (
  ...
  `pay_type` tinyint(1) UNSIGNED NULL COMMENT '支付方式 1：余额支付；2：支付宝；3：微信',
...
CREATE TABLE `notify_task` (
   ...
   `voucher_id` varchar(8) NOT NULL COMMENT '优惠卷ID',
   ...
   `create_time` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
   `update_time` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
   PRIMARY KEY (`id`),UNIQUE KEY `uq_user_id` (`user_id`)
